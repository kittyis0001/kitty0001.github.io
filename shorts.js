...
;(function () {
  ...
  // ── Icon markup (Instagram-Reels-style outline SVGs, currentColor
  // so CSS controls the color; no emoji glyphs). Kept as small
  // constants so buildSlideElement / toggleMute stay readable. ──
  const ICON_SAVE = '<svg viewBox="0 0 24 24" width="26" height="26" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M19 21l-7-5-7 5V5a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2z"/></svg>'
  ...
  const ICON_VOLUME_ON = '<svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"/><path d="M15.54 8.46a5 5 0 0 1 0 7.07"/><path d="M19.07 4.93a10 10 0 0 1 0 14.14"/></svg>'
  const ICON_VOLUME_OFF = '<svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"/><line x1="23" y1="9" x2="17" y2="15"/><line x1="17" y1="9" x2="23" y2="15"/></svg>'
  ...
  function buildSlideElement(short, index) {
    ...
    slide.innerHTML = `
      ...
      <button class="shorts-mute-btn">${userHasUnmuted ? ICON_VOLUME_ON : ICON_VOLUME_OFF}</button>
      ...
      <div class="shorts-actions">
        <button class="shorts-action-btn shorts-save-btn">
          <div class="shorts-action-icon${isSaved ? ' saved' : ''}">${isSaved ? ICON_SAVE_FILLED : ICON_SAVE}</div>
          <span class="shorts-action-label">Save</span>
        </button>
        <button class="shorts-action-btn shorts-share-btn">
          <div class="shorts-action-icon">${ICON_SHARE}</div>
...
// ══════════════════════════════════════════════════════
  ...
  function toggleMute() {
    ...
    document.querySelectorAll('.shorts-mute-btn').forEach(b => { b.innerHTML = userHasUnmuted ? ICON_VOLUME_ON : ICON_VOLUME_OFF })
  ...
  async function toggleSaveShort(index) {
    ...
    if (isSaved) {
      ...
      if (iconEl) { iconEl.classList.add('saved'); iconEl.innerHTML = ICON_SAVE_FILLED }
      try {
